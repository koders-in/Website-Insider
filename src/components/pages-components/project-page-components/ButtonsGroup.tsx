    ...
    <div className="flex flex-wrap gap-3 md:gap-8 items-center font-miligramMedium md:pl-4">
      {buttonsArray.map((item, i) => (
        <Button
          ...
          logo={add}
          hoverLogo={tick}
          hoveLogoStyle="h-[8px] md:h-[12px]"
          logoStyle="h-[8px] md:h-[12px]"
          isSelect={formState?.technologies?.find((elm) => elm === item)}
          className="border-[1px] sm:border-2 text-[0.8rem] sm:text-[1.1rem] border-main-teal rounded-lg px-2 py-1 sm:px-4 sm:py-3 hover:bg-main-teal hover:text-black"
        />
      ))}
    </div>
  );
